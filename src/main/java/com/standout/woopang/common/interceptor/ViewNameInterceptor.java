package com.standout.woopang.common.interceptor;
...
public class ViewNameInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		try {
			...
			int cartCount = 0;
			cartCount=sqlSession.selectOne("mapper.woopang.counts.cartLen",member_id);
			session.setAttribute("cartCount", cartCount);

			int deliveringCount = 0;
			deliveringCount=sqlSession.selectOne("mapper.woopang.counts.deliveringLen",member_id);
			session.setAttribute("deliveringCount", deliveringCount);

//			Long woopang_money = 0L;
//			woopang_money=(Long)sqlSession.selectOne("mapper.woopang.counts.woopang_money",member_id);
//			session.setAttribute("woopang_money", woopang_money);

			//관리자일경우, 상품갯수, 주문건수, 총매출 출력.
			//System.out.println(member_id);
			if(member_id.equals("1111") == true) {
				int goodsLen = 0;
				goodsLen=sqlSession.selectOne("mapper.woopang.counts.goodsLen");
				session.setAttribute("goodsLen", goodsLen);

				int ordersLen = 0;
				ordersLen=sqlSession.selectOne("mapper.woopang.counts.ordersLen");
				session.setAttribute("ordersLen", ordersLen);

				Long totalSales = 0L;
				totalSales=(Long)sqlSession.selectOne("mapper.woopang.counts.totalSales");
